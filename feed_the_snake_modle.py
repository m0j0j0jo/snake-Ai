...
class Feed_The_Snake_Modle(MLModle):
    ...
    def save_mod(self):
        self.model.save(modle_names.feed_the_snake.name + 'model')
    ...
    def load_model_and_train(self):
        self.get_model()
        if os.path.exists(modle_names.feed_the_snake.name + 'model'):
            self.model.load(modle_names.feed_the_snake.name + 'model')
        self.input_data()
        self.model.fit(self.input_X, self.input_Y, n_epoch=1, shuffle=True, run_id=self.filename_feed_the_snake)

    def get_data(self, action):
        return np.append([action], self.generate_observation_feed_the_snake(self.game.snake, self.game.food))
    ...
    def predict_move(self):
        ...
        print('predictions {} choose {}'.format(predictions,move))
        key = self.get_key_from_direction(self.get_snake_direction_vector(self.game.snake), move)
        self.data_feed_the_snake = self.data_feed_the_snake + [list(self.get_data(move)) + [self.calculate_err_feed_the_snake(key)]]
        return key

    def input_data(self):
        input_file = pickle.load(open(self.filename_feed_the_snake, 'rb'))
        temp_x = list()
        tempY = list()
